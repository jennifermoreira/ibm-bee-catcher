...
function startGame(demo = false) {
  ...
  // Show / hide demo badge in HUD
  const demoBadge = document.getElementById('hud-demo-badge');
...
function showResult(score) {
  ...
  // Demo mode notice
  const demoNotice = document.getElementById('result-demo-notice');
  ...
  if (demo) {
    // Demo mode — show what tier they would have won, but award nothing
    ...
    if (score === 0) {
      resultNoPrize.textContent = "No bees caught — keep practising!";
    } else if (won) {
      resultNoPrize.textContent = `Great catch! In a real game you'd win the ${won.name} tier 🎁`;
      setTimeout(() => { launchConfetti(); flashLight(); }, 150);
    } else {
      resultNoPrize.textContent = "So close! A few more bees and you'd win a prize!";
    }
  } else {
    const tiers = [...cfg.tiers].sort((a, b) => b.minBees - a.minBees);
    const won   = tiers.find(t => score >= t.minBees);
    ...
    if (score === 0) {
      ...
      if (assignedGift) {
        ...
        // Won the tier but both gifts are out of stock — still celebrate and show tier
        resultPrize.classList.remove('hidden');
        resultGiftPanel.classList.add('hidden');
        resultNoPrize.classList.add('hidden');
        resultPrize.querySelector('.prize-name').textContent = won.name;
        resultPrize.querySelector('.prize-gift').textContent = 'Please see our booth staff — a replacement gift will be provided!';
        setTimeout(() => {
          playCelebration();
          launchConfetti();
          flashLight();
