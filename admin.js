...
(function attachLongPress() {
  ...
  function _createProgress() {
    ...
    _progressEl.id = 'admin-long-press-ring';
  ...
  function _onStart(e) {
    // Only trigger on the logo area (bee image + title)
    const logoArea = target.querySelector('.logo-area');
    if (!logoArea) return;
    const touch = e.touches ? e.touches[0] : e;
    _startX = touch.clientX;
    _startY = touch.clientY;
    if (!logoArea.contains(e.target)) return;

    e.preventDefault();
    _createProgress();

    // Animate the ring fill over 3s
    requestAnimationFrame(() => {
      if (_progressEl) _progressEl.classList.add('admin-long-press-active');
    });
  ...
  function _onMove(e) {
    ...
    // Cancel if finger moved more than 12px (intentional scroll/swipe)
    if (Math.sqrt(dx * dx + dy * dy) > 12) _onEnd();
  }

  target.addEventListener('touchstart',  _onStart, { passive: false });
  target.addEventListener('touchend',    _onEnd);
  target.addEventListener('touchcancel', _onEnd);
  target.addEventListener('touchmove',   _onMove,  { passive: true });
})();
